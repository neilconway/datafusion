...
### `Box<C>` `TreeNodeContainer` impl now requires `C: Default`

The `TreeNodeContainer` implementation for `Box<C>` now requires `C: Default`.
This change was necessary as part of optimizing tree rewriting to reduce heap
allocations.

**Who is affected:**

- Users that implement `TreeNodeContainer` on a custom type and wrap it in
  `Box` when walking trees.
