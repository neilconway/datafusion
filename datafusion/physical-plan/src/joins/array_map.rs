        ...
        T::Native: Copy + AsPrimitive<u64>,
    {
        ...
        for probe_idx in range.start..end {
            if have_null && arr.is_null(probe_idx) {
                // ArrayMap does not receive NullEquality, but skipping probe
                // nulls is valid for every ArrayMap created by hash join:
                // NullEqualsNothing never matches nulls, and
                // hash_join::exec::try_create_array_map rejects ArrayMap for
                // NullEqualsNull if the build join keys contain nulls.
                continue;
            }
